...
- **"No tracked game matches this launch"** — the game has no `--appid`, so the `savelocker run` wrapper can't match it to a tracked game. Run `savelocker scan` to find the correct AppID, then re-add it: `savelocker add-game --name … --dir … --appid <id>`.
