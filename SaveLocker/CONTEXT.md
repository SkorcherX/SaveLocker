...
4. **Linux agent (Proton / Steam Deck)** — design locked in `Decisions.md`, phased plan in `tasks/linux-agent.md`. **Phase 0 (WSL2 dev env) ✅ done** — Ubuntu 24.04 + .NET 9.0.315 in `~/.dotnet`, repo at `~/SaveLocker` on ext4. **Phase 1 (`Agent.Core` split) ✅ done 2026-07-12** — sync brain is now net9.0 and platform-neutral; Windows bits (`IAutoStart`, `IGameScanner`, folder picker) injected from `TrayApp`. Build clean, 10/10. **Next: Phase 2** — Linux daemon, CLI, Proton path resolution, launch wrapper. One phase at a time.
