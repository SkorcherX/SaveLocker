...
## Medium priority
- **MinVer never matches the git tags (`MinVerTagPrefix` unset)** — tags are `v0.1.5`, but MinVer's default tag prefix is **empty**, so it searches for a tag literally named `0.1.5`, finds none, and falls back to `MinVerMinimumMajorMinor` → every local build stamps **`0.1.0`** instead of `0.1.6-alpha.N`. Harmless *today* only because `build-installer.ps1` and `release.yml` both pass the version explicitly (`MinVerVersionOverride` / `--property:Version`), which masks it. Fix: `<MinVerTagPrefix>v</MinVerTagPrefix>` in `SaveLocker.Agent.csproj`, then confirm an untagged build reports `0.1.6-alpha.*` and a tagged one still reports the tag.
